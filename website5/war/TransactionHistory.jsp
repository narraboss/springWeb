					...
					<tr>
						...
						<td><select name="accno">
								...
								<%
									...
									for (Account acc : a) {
										if ((acc.getCustomerId()).equals((String) session.getAttribute("customerId").toString())) {
								%>
						...
						<tbody>
							...
							<%
								...
								for (Transaction t1 : date) {
									if (t1.getDateOfTransaction().after((Date) session.getAttribute("sdate"))
											&& t1.getDateOfTransaction().before((Date) session.getAttribute("edate"))) {
										trans.add(t1);
