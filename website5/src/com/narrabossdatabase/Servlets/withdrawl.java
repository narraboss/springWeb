...
public class withdrawl extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		if (admin_LoginServlet.admin_login == 1) {
			...
			resp.getWriter().println("<a href=\"/JSPfiles/Withdrawl.jsp\">"
					+ "<h3 style=\"color:#1975D1\">Back</h3></a>" + "</body></html>");
		} else
		...
		for (Account acc : a) {
			if ((acc.getAccountNo()).equals(f)) {
				...
				if (Double.parseDouble(acc.getBalance()) > Double.parseDouble(s)) {
					...
					TransactionDao.INSTANCE.add(acc.getCustomerId(), acc.getAccountNo(), new Date(), null, amount,
							toBal, null, "Withdrawl");
