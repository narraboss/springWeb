...
public class MiniStatement extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		if ((session.getAttribute("login").toString()).equals("1")) {
			...
			session.setAttribute("minacc", accountNo);
			 ...
			 * accountNo.equals(t.getAccountNo())) {
			 * 
			 * date.add(t); } Collections.sort(date, new
			 * Comparator<Transaction>() { public int compare(Transaction o1,
			 * Transaction o2) { return
			 * o1.getDateOfTransaction().compareTo(o2.getDateOfTransaction()); }
			 * });
			 * 
			 * 
			 * Collections.reverse(date); session.setAttribute("date", date);
			 * 
			 * }
			 */
